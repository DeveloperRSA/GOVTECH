...
## 8. Merge log — Organisation, Workspace & Reviewer brought in

A second project (with a newer Organisation page, a real Workspace/Document
Repository, and Reviewer screens) was merged in on top of this one. Rule
used throughout: **Funding Agreement, Case and Accountability Case stayed
exactly as they were** (protected); everything else was evaluated and
merged deliberately rather than either project winning by default.

### What was brought in

| File | What it adds |
|---|---|
| `app/dsac/organisations.jsx` (replaces the old `.tsx`) | Organisation registration **and** first-admin account creation with a password, calling the `create-organisation-admin` edge function |
| `app/organisation/dashboard.jsx` (replaces the old `.tsx`) | A real organisation-side dashboard: membership lookup, team list, org details — the old version was a placeholder ("modules will appear here as they're added") |
| `app/organisation/workspace.jsx` (new) | A genuine Document Repository: file upload to Supabase Storage, version history (`documents` + `document_versions`), Microsoft Word/Excel/PowerPoint launch links, audit logging |
| `app/reviewer/dashboard.jsx`, `app/reviewer/organisation-reviews.jsx` (new) | The reviewer role now has somewhere to land — resolves the "no reviewer dashboard" gap flagged in §5 previously |
| `app/auth/login.jsx` | Adopted for inline field-validation error messages (nice-to-have, low risk — flag-colored borders stripped to match the rest of the app, same as the earlier cleanup pass) |
| `supabase/config.toml`, `supabase/functions/create-organisation-admin/` | Edge function scaffold — **rewritten**, see below |

### Conflicts found and how they were resolved (not blindly picked)

- **`dashboard.jsx` case-count bug, present in *both* versions.** The
  "Accountability Cases" stat filtered by status values (`ASSIGNED`,
  `SUBMITTED`, `RESUBMITTED`) that `cases.jsx` — in either project — never
  actually writes; it writes `DRAFT` / `IN PROGRESS` / `UNDER REVIEW` /
  `ACTION REQUIRED` / `APPROVED`. Took the newer responsive layout but
  fixed the filter to match what `cases.jsx` actually produces, otherwise
  the stat would have quietly undercounted.
- **`create-organisation-admin` edge function was an unfilled template**
  — still the default `Hello ${name}!` scaffold Supabase generates, even
  though the Organisation page already calls it expecting it to create a
  user. Rewrote it for real (see below) rather than shipping a page whose
  core feature silently does nothing.
- **A genuine syntax bug in `app/organisation/dashboard.jsx`** (inherited,
  not introduced by the merge): a duplicated, malformed nav item and a
  duplicated breadcrumb block that didn't close correctly — the file
  wouldn't have compiled at all. Fixed both; verified with a full
  Babel parse of every `.jsx`/`.tsx`/`.js`/`.ts` file in `app/` and `src/`.

### `create-organisation-admin` — real implementation

The edge function now: verifies the caller is signed in **and** an active
`DSAC_ADMIN` (checked server-side, independent of the frontend), creates
the auth user with the service role, inserts their `user_profiles` row
(`role: 'ORG_ADMIN'`), links them via `organisation_memberships`
(`membership_role: 'ORG_ADMIN'`, `status: 'ACTIVE'`), and rolls back the
auth user if either insert fails so you never end up with an orphaned
login that has no profile. Deploy it with:

```bash
supabase functions deploy create-organisation-admin
```

It relies on `SUPABASE_URL`, `SUPABASE_ANON_KEY` and
`SUPABASE_SERVICE_ROLE_KEY`, which Supabase injects into every deployed
edge function automatically — nothing to configure.

### Schema additions (`sql/schema.sql`)

None of `organisation_memberships`, `documents`, `document_versions`,
`reviews`, or `audit_logs` were defined in either project's zip — the
frontend assumes they exist. `sql/schema.sql` now scaffolds all five with
starter RLS policies. **If your live Supabase project already has some of
these** (this app has clearly been run against a real database before —
the organisation dashboard and workspace code wouldn't make sense
otherwise), reconcile column names against what's there rather than
running the script blind; it's written with `if not exists` guards so
it's safe to run, but it can't know about columns you already added that
this script doesn't know about.

Also needed once, not scriptable in SQL: a **Storage bucket named
`documents`** (Workspace uploads files there) — see §5 of `sql/schema.sql`
for the exact commands.

### `package.json`

Added `@expo/vector-icons` and `expo-document-picker` — both used by the
new Workspace page and missing from the existing dependency list. Run
`npm install` after pulling this down.

### Known gaps carried over from the source project (not introduced here)

- `reviewer/dashboard.jsx` links to `/reviewer/funding-reviews` and
  `/reviewer/accountability-reviews`, and to a `/reviewer/reviews/[id]`
  detail route — **none of these pages exist** in either project. They'll
  hit Expo Router's "Unmatched Route" screen rather than crash, but they're
  dead links until those screens are built.
- Documents aren't scoped to an organisation yet — `workspace.jsx`'s own
  comment says so (`"There is currently NO organisation_id on documents.
  Therefore documents are loaded by created_by."`). Every signed-in user
  currently sees only documents *they* uploaded, not their organisation's.
  Worth fixing before this goes further; left as-is since it's a frontend
  change to a page brought in wholesale from the other project, not
  something to redesign mid-merge.

### Verified after merging

- Every `.jsx`/`.tsx`/`.js`/`.ts` file under `app/` and `src/` parses
  cleanly (Babel, `babel-preset-expo`) — 23 files, 0 failures.
- `cases.jsx`, `funding-agreements.jsx`, and `analytics.jsx` are
  byte-for-byte identical to the pre-merge versions (diffed to confirm).
- Dashboard nav's dead "REPORTS" item now points at `/dsac/analytics`
  (same fix as before, reapplied since the whole file was replaced); a
  fourth quick-action card links there too.
- `app/index.js` now sends `DSAC_REVIEWER` to the real `/reviewer/dashboard`
  instead of the `/dsac/dashboard` stopgap from before.

**Not yet done — needs a live environment to check:** actually running
`npm install` and the app end-to-end, since this container can parse and
reason about the code but can't run Expo/Metro or hit a real Supabase
project. Test the flow in §9 of the merge instructions before treating
this as final: Organisation → set password → Workspace → Funding
Agreement → Case → Accountability Case.
